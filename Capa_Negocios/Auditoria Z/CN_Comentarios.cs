...
namespace Capa_Negocios.Auditoria_Z
{
    public class CN_Comentarios
    {
        ...
        public bool RegistrarComentario(Capa_Entidad.Auditoria_Z.E_Comentarios obj, IFormFile? archivo, string webRootPath)
        {
            // 1. Si hay un archivo, procesamos el guardado físico primero
            if (archivo != null && archivo.Length > 0)
            {
                string folderPath = Path.Combine(webRootPath, "uploads", "auditoriaZcomentarios");
                ...
                obj.RutaArchivo = "/uploads/auditoriaZcomentarios/" + nombreSistema; // Ruta relativa para la web
            }
        ...
        public List<Capa_Entidad.Auditoria_Z.E_Comentarios> Listar(int idCanje)
        {
            return _daoComentarios.ListarPorAuditoriaZ(idCanje);
        }
    }
}
