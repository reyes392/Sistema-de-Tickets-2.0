...
namespace Capa_Datos.Anulaciones
{
    public class CD_Archivo
    {
        ...
        public CD_Archivo(IConfiguration configuration)
        {
        ...
        public bool Registrar(E_Archivos obj)
        {
            ...
            {
                string query = @"INSERT INTO ARCHIVOS_ADJUNTOS_ANULACIONES (ID_ANULACION, NOMBRE_ARCHIVO, NOMBRE_SISTEMA, EXTENSION, RUTA_SERVIDOR) 
        ...
        public List<E_Archivos> ListarPorAnulacion(int idAnulacion)
        {
            var lista = new List<E_Archivos>();
            using (var cn = new SqlConnection(_connectionString))
            {
                string query = "SELECT NOMBRE_ARCHIVO, RUTA_SERVIDOR, EXTENSION FROM ARCHIVOS_ADJUNTOS_ANULACIONES WHERE ID_ANULACION = @id";
                SqlCommand cmd = new SqlCommand(query, cn);
