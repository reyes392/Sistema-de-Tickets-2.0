...
namespace Capa_Datos.ReclamosBodega
{
    public class CD_Archivos
    {
        ...
        public bool Registrar(E_Archivos obj)
        {
            ...
            {
                string query = @"INSERT INTO ARCHIVOS_ADJUNTOS_RECLAMOS (ID_RECLAMO, NOMBRE_ARCHIVO, NOMBRE_SISTEMA, EXTENSION, RUTA_SERVIDOR) 
        ...
        public List<E_Archivos> ListarPorTicket(int idReclamo)
        {
            var lista = new List<E_Archivos>();
            using (var cn = new SqlConnection(_connectionString))
            {
                string query = "SELECT NOMBRE_ARCHIVO, RUTA_SERVIDOR, EXTENSION FROM ARCHIVOS_ADJUNTOS_RECLAMOS WHERE ID_RECLAMO = @id";
                SqlCommand cmd = new SqlCommand(query, cn);
